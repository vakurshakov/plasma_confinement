...
// is for one particle, in the cicle we will do an addition
void Esirkepov_density_decomposition(const sort_of_particles& soof_particles,
									 const particle& PARTICLE,
									 const vector2& r0,
									 vector3_field& j,
									 double dt, double dx, double dy)
{
	double gamma = sqrt(1 + PARTICLE.p().dot(PARTICLE.p())/(soof_particles.m()*soof_particles.m()));
	double vz = PARTICLE.p().z()/gamma;
	vector2 r = PARTICLE.r();
	double q  = soof_particles.q();
	int charge_cloud = soof_particles.charge_cloud();
	double (*shape_at)(double, double) = soof_particles.form_factor();
	...
	#pragma omp parallel sections shared(j, temp_j, r, vz, q, nearest_edge_to_rx, nearest_edge_to_ry, charge_cloud, dx, dy), num_threads(3)
	{
		#pragma omp section
		{	// collecting jx(y,x) -------------------------------------------------------------------------
			...
			for (long int x = -charge_cloud, y = -charge_cloud; y <= +charge_cloud; ++y) {
				...
				temp_j.x(y,x) = - q*0.5*dx/dt*(shape_at(r.x() - node_x*dx, dx) - shape_at(r0.x() - node_x*dx, dx))*
			...
			for (long int y = -charge_cloud; y <= +charge_cloud; ++y) {
				...
				for (long int x = -charge_cloud+1; x <= +charge_cloud; ++x) {
					...
					temp_j.x(y,x) = temp_j.x(y,x-1) - q*0.5*dx/dt*(shape_at(r.x() - node_x*dx, dx) - shape_at(r0.x() - node_x*dx, dx))*
																  (shape_at(r.y() - node_y*dy, dy) + shape_at(r0.y() - node_y*dy, dy));
		...
		{	// collecting jy(y,x) -------------------------------------------------------------------------
			...
			for (long int y = -charge_cloud, x = -charge_cloud; x <= +charge_cloud; ++x) {
				...
				temp_j.y(y,x) = - q*0.5*dy/dt*(shape_at(r.x() - node_x*dx, dx) + shape_at(r0.x() - node_x*dx, dx))*
											  (shape_at(r.y() - node_y*dy, dy) - shape_at(r0.y() - node_y*dy, dy));
			...
			for (long int y = -charge_cloud+1; y <= +charge_cloud; ++y) {
				...
				for (long int x = -charge_cloud; x <= +charge_cloud; ++x) {
					...
					temp_j.y(y,x) += temp_j.y(y-1,x) - q*0.5*dy/dt*(shape_at(r.x() - node_x*dx, dx) + shape_at(r0.x() - node_x*dx, dx))*
																   (shape_at(r.y() - node_y*dy, dy) - shape_at(r0.y() - node_y*dy, dy));
		...
		{	// collecting jz(y,x) -------------------------------------------------------------------------
			...
			for (long int y = nearest_edge_to_ry - charge_cloud; y <= nearest_edge_to_ry + charge_cloud; ++y) {
				...
				j.z(y,x) += - q*vz/3.*((	shape_at(r.x() - x*dx, dx) + 0.5*shape_at(r0.x() - x*dx, dx))*shape_at(r.y() - y*dy, dy)+
