				  ...
				  double dt, double dx, double dy)
{
	...
	for(int x = nearest_edge_to_rx-charge_cloud; x <= nearest_edge_to_rx+charge_cloud; ++x) {
		...
		for(int y = nearest_edge_to_ry-charge_cloud; y <= nearest_edge_to_ry+charge_cloud; ++y) {
			...
			local_E.x() += E.x(y,x)*shape[shifted].y()*shape[noshift].x();
			local_E.y() += E.y(y,x)*shape[noshift].y()*shape[shifted].x();
			local_E.z() += E.z(y,x)*shape[shifted].y()*shape[shifted].x();

			local_B.x() += B.x(y,x)*shape[noshift].y()*shape[shifted].x();
			local_B.y() += B.y(y,x)*shape[shifted].y()*shape[noshift].x();
			local_B.z() += B.z(y,x)*shape[noshift].y()*shape[noshift].x();	
		}
	}
